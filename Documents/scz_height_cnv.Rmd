...
copies so this is a product 
of the frequency and the number of copies).  More likely than this is that the height CNVs 
are given as a percentage not as a raw frequency.



```{r scz_frequencies, cache=TRUE, echo=FALSE}
scz <- rbind( 
  data.frame(case=gscz.del$fcase, control=gscz.del$fcontrol, type="del"),
  data.frame(case=gscz.dup$fcase, control=gscz.dup$fcontrol, type="dup")
)
ggplot(scz, aes(x=control, y=case, col=type)) + geom_point(alpha=0.02) + guides(colour = guide_legend(override.aes = list(alpha = 1)))
```

### Tabulate frequencies

```{r table_freq, echo=FALSE}
...
ggplot(tbf, aes(y=Freq, x=cat, col=cohort)) + geom_point() +facet_wrap(~type)
...
```{r create_comparison_datasets, echo=FALSE}
...
ggplot(height_scz.dup, aes(x=fdup/100, y=fcontrol)) + geom_point(alpha=0.02) + xlim(0,0.012) + ylim(0,0.012) + xlab("Height Duplication CNV Frequency") + ylab("SCZ Duplication Frequency") +ggtitle("Duplication Frequencies") + guides(colour = guide_legend(override.aes = list(alpha = 1))) + facet_wrap(~distance)

```

```{r table_scz_matches_dup}
pander(table(distance = height_scz.dup$distance))
```
I can cut down the number of positions to look at, and get a set of CNVs that more cosely match the frequencies in 
both datasets by just choosing those within 100 bases.  This leave `r sum (height_scz.dup$close)`
...
height_scz.dup <- height_scz.dup[height_scz.dup$close,]
pander(table(cut(height_scz.dup$start_h-height_scz.dup$start_s, breaks=c(-100,-50,-10,-1,0,1,10,50,100))))
...
ggplot(height_scz.dup, aes(x=beta_height, y=z, col=h_col)) + geom_point(alpha=0.1) + theme(legend.title=element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1))) + ggtitle("Effect Sizes for Height vs SCZ")


p1 <- ggplot(height_scz.dup, aes(x=fcase, y=fcontrol, col=h_col)) + geom_point(alpha=0.05) + theme(legend.title=element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1))) 
p2 <-ggplot(height_scz.dup, aes(x=fcase, y=fcontrol, col=bmi_col)) + geom_point(alpha=0.05) + theme(legend.title=element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1))) 
...
test_region <- GRanges(seqnames=height_scz.dup$seqnames[l[1]], 
                       IRanges(start=height_scz.dup$start_h[l[1]], end=height_scz.dup$end[l[length(l)]])) 

plot_region <- test_region + 10000000

height_region <- subsetByOverlaps(gheight_cnv, plot_region)
scz_del_region <- subsetByOverlaps(gscz.del, plot_region)


plot(start(height_region), height_region$beta_height)

testloc <- height_scz.dup[l2,]

plot(testloc$start_h, testloc$fcontrol)
lines(testloc$start_h, testloc$fcase, type="l")


plot(testloc$start_h, testloc$fcontrol)
plot(testloc$fdel, testloc$fdup)
abline(0,1)
...
#l <- identify(height_scz.dup$fcontrol, height_scz.dup$fcase)
...
plot(testloc$start_h, testloc$fcontrol, ylim=c(0,max(testloc$fcase, testloc$fcontrol)), type="l")
lines(testloc$start_h, testloc$fcase, type="l", col="blue")
lines(testloc$start_h, testloc$fdup/100, type="l", col="green")
lines(testloc$start_h, testloc$fdel/100, type="l", col="red")

plot(testloc$start_h, testloc$beta_height, col=1, ,ylim=c(range(c(testloc$beta_height,testloc$z))))
points(testloc$start_h, testloc$z, col=2)

```

What I probably want to do here is to go back to the raw data for both the 
height and scz datasets and see what this does to fill in the gaps in these data
...
ggplot(height_scz.del, aes(x=fdel/100, y=fcontrol)) + geom_point(alpha=0.02) + xlim(0,0.012) + ylim(0,0.012) + xlab("Height Deletion CNV Frequency") + ylab("SCZ Deletion") +ggtitle("Deletion") + guides(colour = guide_legend(override.aes = list(alpha = 1))) + facet_wrap(~distance)
...
ggplot(height_scz.del, aes(x=fcontrol, y=fcase, col=bmi_col)) + geom_point(alpha=0.05) + theme(legend.title=element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1))) 


plot(height_scz.del$fcontrol,height_scz.del$fcase, col=height_scz.del$h_col)
l_del <- identify(height_scz.del$fcontrol,height_scz.del$fcase)
l_del <- c(12730,12731,12735,12745,12751,12752,12755,12762,12773,12793,12796,12797)
del_check <- height_scz.del[l_del,]

ggplot(height_scz.del, aes(x=fcase, y=fcontrol, col=bmi_col)) + geom_point(alpha=0.05) + theme(legend.title=element_blank()) + guides(colour = guide_legend(override.aes = list(alpha = 1))) +facet_wrap(~cut_cnvfreq)


plot(height_scl_del_beta <- identify(height_scz.del$beta_height,height_scz.del$z)
z.del$beta_height,height_scz.del$z, col=height_scz.del$h_col)
l_del_beta <- identify(height_scz.del$beta_height,height_scz.del$z)
l_del_beta <- c(192,194,201,203,205,209,210,2322,2323,2324,2332,2333,2334,2336,2337,2338,2339,9571,9582,9587,9590,9592,9594,9600,9602,9604)
...
plot(height_scz.del$fcontrol,height_scz.del$fcase, col=height_scz.del$bmi_col)
l_del <- identify(height_scz.del$fcontrol,height_scz.del$fcase)
plot(height_scz.del$fcontrol,height_scz.del$fcase, col=height_scz.del$bmi_col)
l_del <- c(12730,12731,12735,12745,12751,12752,12755,12762,12773,12793,12796,12797)
del_check <- height_scz.del[l_del,]
...
plot_subset_regions <- function(id, target, ex=2000000) {
  ...
  yrange <- range(height_region$beta_height, scz_del_region$z, scz_dup_region$z)
...
So there apppears to be a tendency for CNVs that are associated with one to be the same as SNPs that are 
associated with the other.


### Can do the same with SNPs
