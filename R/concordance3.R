...
load(here("output", "snps_SHN.rda"))

#' ## Simplfy things to get some simple statistics and plots

z <- data.frame(scz = snp_shn$zb.scz, height=snp_shn$zb.height, neur = snp_shn$zb.neu) 
...
colnames(gene_m) <- c("scz", "height", "neur")
...
o <- as.vector(t(outer(c("s","","S"), c("h", "", "H"), paste0)))
